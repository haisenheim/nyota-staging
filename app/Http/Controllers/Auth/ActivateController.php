...
class ActivateController extends Controller
{
    ...
    private static $usersHomeRoute ='activate-success.home';
    ...
    public static function getUsersHomeRoute()
    {
        return self::$usersHomeRoute;
    }
    ...
    public function activate($token)
    {
        ...
        if($user){
			...
			$datauid = Activation::where('token', $token)->first();
			$user = User::where('id',$datauid->user_id)->first();
		}
		$currentRoute = Route::currentRouteName();
        ...
        if($user->isuser()){
            return redirect()->route(self::getUsersHomeRoute());
        }
        return redirect()->route(self::getUserHomeRoute())
            ->with('status', 'success')
            ->with('message', trans('auth.successActivated'));
