    ...
    <div class="box box-primary">
      ...
      <div class="row row-box">
        ...
        <div class="box-tools pull-left col-lg-2">  
          <a class="btn btn-primary btn-sm" href="{{ URL::to('/admin/products/create') }}">
            Create
          </a>
        </div>
      ...
      <div class="col-lg-4">
        <div class="row">
          <form action="{{ route('blukproduct') }}" id="importproduct" method="POST" enctype="multipart/form-data">
            @csrf
      ...
      <div class="col-lg-6">
              <div class="row">
                {!! Form::open(['route' => 'search-products','method' => 'get', 'role' => 'form', 'class' => 'needs-validation', 'id' => 'search_pages']) !!}
                 {!! csrf_field() !!}
                      ...
                      @foreach($products as $product)
                     	<tr>
                          ...
                          <td>@if($product->is_active == 0)
                            ...
                            @endif</td>
                          <td>
                            {!! Form::open(array('url' => 'admin/products/' . $product->id, 'class' => '', 'data-toggle' => 'tooltip')) !!}
                            ...
                            {!! Form::close() !!}
                            <a class="btn btn-sm btn-info" href="{{ URL::to('/admin/products/' . $product->id . '/edit') }}" data-toggle="tooltip">
                            <i class="fa fa-edit"></i>
                            </a>
                            <a class="btn btn-sm btn-primary" href="{{ URL::to('/admin/products/' . $product->id ) }}" data-toggle="tooltip"><i class="fa fa-eye"></i></a>
                            <a class="btn btn-sm btn-warning" href="{{ URL::to('/admin/products/addcomment/' . $product->id ) }}" data-toggle="tooltip">Comment</a>
    ...
    var url = form.attr("action");
     ...
     $.ajax({
        ...
        success: function (data) {
          ...
          if (data.fail) {
            //var error_length = data.errors;
            var len = data.errors.length;
            for(var i=0; i<len; i++){
              var message = data.errors[i];
              var li_str = "<li>" + message + "</li>";
                  $('.alert').show();
                  $('.error_alert').append(li_str);
            }
